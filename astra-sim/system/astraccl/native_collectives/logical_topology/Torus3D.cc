...
Torus3D::Torus3D(int id, int total_nodes, int local_dim, int vertical_dim) {
    ...
    local_dimension = new RingTopology(RingTopology::Dimension::Local, id,
                                       local_dim, id % local_dim, 1);
    vertical_dimension = new RingTopology(
        RingTopology::Dimension::Vertical, id, vertical_dim,
        id / (local_dim * horizontal_dim), local_dim * horizontal_dim);
    horizontal_dimension = new RingTopology(
        RingTopology::Dimension::Horizontal, id, horizontal_dim,
