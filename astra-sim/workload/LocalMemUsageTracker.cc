...
void LocalMemUsageTracker::buildTensorLifetimeHeatmap() {
    ...
    auto getSizeColor = [](uint64_t size, uint64_t maxSize) -> std::string {
        ...
        // int intensity = std::min(255,
        // static_cast<int>((static_cast<double>(size) / maxSize) * 255));
        const double ratio = (maxSize == 0) ? 0.0
                                            : static_cast<double>(size) /
                                                  static_cast<double>(maxSize);
        const int intensity =
            std::clamp(static_cast<int>(ratio * 255.0), 0, 255);
        char color[8];
        //sprintf(color, "#%02X%02X%02X", intensity, 100, 255 - intensity);
        std::snprintf(color, sizeof(color), "#%02X%02X%02X", intensity, 100, 255 - intensity);
